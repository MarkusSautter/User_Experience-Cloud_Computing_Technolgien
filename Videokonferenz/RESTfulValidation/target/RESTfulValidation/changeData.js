...
//		});
	...
	chartRow.transition()
		...
		newRow.on("click", function() {	
			getDoc();
			historyJSON =newRow.text();
			//document.getElementById("videoKonferenz").style.visibility = "visible";
			minVideo();
		});
};
...
var pullData = function(settings,callback){
	...
	$.get("DocumentsService", function(response) {   // Execute Ajax GET request on URL of "someservlet" and execute the following function with Ajax response text...
		//$("#testDIV").html(responseText);           // Locate HTML DOM element with ID "somediv" and set its text content with the response text.
		//responseText = 'hierda';
		//$('#test2').text(response);
		json = response;
	});
	...
	d3.json("JSONS/Docs1.json", function (err, data){
...
//		newData = d3.json("JSONS/Docs2.json");
//		var test = JSON.stringify(newData);
//		window.alert(test);
//		d3.json("JSONS/Docs2.json", function(data) {
//			  console.log(data[0]);
//			  window.alert(data);
//			});
		json = JSON.parse(json);
		//window.alert(json);
		newData = formatData(json);
